...
def generate_biological_jacobian(NR_, NS_):
    D = np.diag(np.random.uniform(low=0, high=1., size=NR_))
    G = build_biological_Gamma(NR_, NS_)
    B = np.random.uniform(low=0., high=1., size=(NS_,NR_))
    Z = np.zeros((NS_,NS_))
...
def generate_feasible_jacobian(NR_, NS_, param):
    ...
    for i in range(NR_):
        ...
        sumterm = 0.
        for j in range(NS_):
            sumterm+=gamma[j][i]*Req[i]*Seq[j]+
            
        
    
    while(not(physical_parameters(delta, mu))):
        for i in range
    

def physical_parameters(delta_, mu_):
    for d in delta_:
        if d < 0 : 
            return False
    for m in mu_:
