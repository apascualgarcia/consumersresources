...
\end{equation}
So $\Delta_S^*(m, G, A)$ can be found by computing $P_E(\Delta_S, m, G, A)$ over the range $[0,1]$ and finding at which point it is equal to 0.5. In practice, a general solver involving methods from the C++ \code{GSL} library was implemented in order to get a good estimate on $\Delta_S^*(m, G, A)$ (see Appendix \ref{app: results structural stability procedure to estimate critical structural perturbation}).
