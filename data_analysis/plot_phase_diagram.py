...
def plot_individual_stability_graphs(data, string_title_, save_extension):
    NR = data[:, 0]
    NS = data[:, 1]
...
effmodel = np.loadtxt(data_folder+'/'+filename+'.out_eff')

plot_individual_stability_graphs(fullmodel, title+' (full model)', 'fullmod')
plot_individual_stability_graphs(effmodel, title+' (effective model)', 'effmod')
