...
def plot_optimized_mat_data(data, fig_to_plots):
    if 'nestG-nestA' in fig_to_plots:
        ...
        xleft = nestmin-0.1*(nestmax-nestmin)
        xright = nestmax+0.1*(nestmax-nestmin)
        xlim = (xleft, xright)
        for i in range(len(cf.all_connectance)):
            ...
            to_plot.plot(x = '<nestG>', y = '<nestA>', yerr='nestA std',label=r'$'+str(connG)+'$', ax=ax1, xlim = xlim, color=cf.conn_colours[i])
        ax1.plot(np.linspace(xleft, xright), np.linspace(xleft, xright), linestyle='--', color='black', marker='')
        ax1.set_xlabel(cf.labels['nestG'])
        ax1.set_ylabel(cf.labels['nestA'])
        ax1.legend(bbox_to_anchor=(1., 1.), loc='upper left', title='Consumption \n connectance '+r'$\kappa_G$', fontsize=13, title_fontsize=14)
        fig1.tight_layout()
    ...
    if 'connG-connA' in fig_to_plots:
        ...
        xleft = connmin-0.1*(connmax-connmin)
        xright = connmax+0.1*(connmax-connmin)
        xlim = (xleft, xright)
        for i in range(len(cf.all_nestedness)):
            ...
            to_plot.plot(x='<connG>', y='<connA>', yerr='connA std', label=r'$'+str(nestG)+'$', xlim=xlim, ax=ax2, color=cf.nest_colours[i])
        ax2.plot(np.linspace(xleft, xright), np.linspace(xleft, xright), linestyle='--', color='black', marker='')
        ax2.set_xlabel(cf.labels['connG'])
        ax2.set_ylabel(cf.labels['connA'])
        ax2.legend(bbox_to_anchor=(1., 1.), loc='upper left', title='Consumption \n overlap '+r'$\eta_G$', fontsize=13, title_fontsize=14)
        fig2.tight_layout()
    ...
    if 'connG-E' in fig_to_plots:
        ...
        ax3 = fig3.add_subplot(111, aspect='auto')
        connmin = np.min(data['<connG>'])
        connmax = np.max(data['<connG>'])
        ...
        for i in range(len(cf.all_nestedness)):
            ...
            to_plot = data.loc[indices]
            to_plot = to_plot.sort_values(by='<connG>')
            to_plot.plot(x='<connG>', y='<E>', yerr='E std', label=r'$'+str(nestG)+'$', xlim=xlim, ax=ax3,  color=cf.nest_colours[i])
        ax3.set_xlabel(cf.labels['connG'])
        ax3.set_ylabel(cf.labels['E'])
        ax3.legend(bbox_to_anchor=(1., 1.), loc='upper left', title='Consumption \n overlap '+r'$\eta_G$', fontsize=13, title_fontsize=14)
        fig3.tight_layout()
    ...
    if 'nestG-E' in fig_to_plots:
        ...
        ax4 = fig4.add_subplot(111, aspect='auto')
        nestmin = np.min(data['<nestG>'])
        nestmax = np.max(data['<nestG>'])
        ...
        for i in range(len(cf.all_connectance)):
            ...
            to_plot.plot(x='<nestG>', y='<E>', yerr='E std', label=r'$'+str(connG)+'$', ax=ax4, xlim=xlim, color=cf.conn_colours[i])
        ax4.set_xlabel(cf.labels['nestG'])
        ax4.set_ylabel(cf.labels['E'])
        ax4.legend(bbox_to_anchor=(1., 1.), loc='upper left', title='Consumption \n connectance '+r'$\kappa_G$', fontsize=13, title_fontsize=14)
        fig4.tight_layout()
