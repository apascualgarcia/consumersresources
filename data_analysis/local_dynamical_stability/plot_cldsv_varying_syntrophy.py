...
for k in range(len(local_dynamical_stability_region)):
    ...
    ax.set_title(r'$\mathcal{D}_{L,1}^*$ for $N_R='+str(int(NR[0]))+', N_S='+str(int(NS[0]))+'$ '+label[k])
    fig.tight_layout()
    fig.savefig("plots/common_local_dynamical_stability_volume_NR"+str(int(NR[0]))+'_NS'+str(int(NS[0]))+'_varying_syntrophy_'+alpha_mode[k]+'.pdf')
    plt.close(k)
