...
for index_matrix in range(0, 26):
    pmin = 0.98
    ...
    cbar.set_label(r'$\mathcal{D}_{L}\left((\gamma_0, S_0, \alpha_0), G, A\right)$')
    title = r'$N_R='+str(NR)+', N_S='+str(NS)+', \eta_G='+str(round(nest,2))+', \kappa_G='+str(round(conn,2))+'$ '+str(label[index_alpha_mode])
    savename ='NR'+str(NR)+'_NS'+str(NS)+'_Nest'+str(nest)+'_Conn'+str(conn)
    fig.savefig('plots/probability_dynamical_stability_'+savename+'_wo_title.pdf')
    fig.suptitle(title)
