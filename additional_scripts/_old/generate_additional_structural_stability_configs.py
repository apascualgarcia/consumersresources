...
# filename='structural_stability/critical_dynamical_syntrophies_NR50_NS25_RS_noIS_conn'
# folder_mode='random_structure_connectance_of_no_release_when_eat_Nr50_Nc25'

# filename='structural_stability/critical_dynamical_syntrophies_NR50_NS25_noIS_gamma_conn'
# folder_mode='no_intraspecific_syntrophy_connectance_of_consumption_mat_Nr50_Nc25'

filename='structural_stability/critical_dynamical_syntrophies_NR50_NS25_LRI_noIS_conn'
...
critical_alpha = np.genfromtxt(filename+'.out', usecols=[1])
...
for i in range(len(matrix_name)):
    ...
    filename='strstab_Nest'+str(nest[i])+'_Conn'+str(conn[i])+'_'+alpha_mode+'_'+folder_mode
    ...
    to_add = []
    to_add.append('alpha0='+str(critical_alpha[i]))
    to_add.append('alpha_mode='+alpha_mode)
    to_add.append('path_to_food_matrix='+matrix_name[i])
    to_add.append('path_to_save_file='+path_to_save)
    to_add.append('path_to_syntrophy_matrix=optimal_matrices/syntrophy/'+folder_mode)
...
common_alpha0 = 0.00307404025011449
for i in range(len(matrix_name)):
    # first create appropriate file
    filename='strstab_Nest'+str(nest[i])+'_Conn'+str(conn[i])+'_'+alpha_mode+'_'+folder_mode
